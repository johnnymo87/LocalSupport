class BaseOrganisation < ActiveRecord::Base
  ...
  scope :what_they_do,  -> { joins(:categories).merge(Category.what_they_do).uniq }
  scope :who_they_help, -> { joins(:categories).merge(Category.who_they_help).uniq }
  scope :how_they_help, -> { joins(:categories).merge(Category.how_they_help).uniq }
